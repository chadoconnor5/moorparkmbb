...
def page_player_profiles():
    ...
    for idx, name in enumerate(profiled):
        ...
        # Archetype row — blend: Synergy archetype when present (richest, works
        # without advanced data); otherwise a box-score offensive role for players
        # who have advanced metrics. Position/usage labels above are the final fallback.
        arch = synergy_arch.get(name)
        box_role, box_summ = (None, None) if arch else box_offensive_role(pos, adv, tot)
        if arch or box_role:
            if arch:
                ...
                arch_cells = [Paragraph(
                    f'<b>Archetype:</b>  {box_role} <font color="#8899aa">(box-score profile)</font>',
                    S(8.5, False, GOLD))]
                arch_cells.append(Paragraph(box_summ, S(7.5, False, colors.HexColor('#aab8d0'))))
