...
totals    = team_sum.get('totals', {})
opp_totals = team_sum.get('opponent_totals', {})
ta        = adv_json['team']
...
for conf in os.listdir(f'{BASE}/2025-26 Team Statistics'):
    ...
    for team in os.listdir(cp):
        ...
        try:
            ...
            _av     = s['averages']
            _opp    = s['opponent_averages']
            _tot    = s.get('totals', {})
            _otot   = s.get('opponent_totals', {})
            _to     = _av.get('TO', 0)
            _ostl   = _opp.get('STL', 0)
            _poss   = max(a.get('possessions', 0.01), 0.01)
            _ast_den = max(_av.get('FGA', 0) + 0.475 * _av.get('FTA', 0) + _av.get('AST', 0) + _av.get('TO', 0), 1)
            _opp_poss_est = max((_otot.get('FGA', 0) - _otot.get('OREB', 0)) + _otot.get('TO', 0) + 0.475 * _otot.get('FTA', 0), 0.1)
            _stl_pct = round(_tot.get('STL', 0) / _opp_poss_est * 100, 1)
            _blk_pct = round(_tot.get('BLK', 0) / max(_otot.get('FGA', 1), 1) * 100, 1)
            _pf_total = _tot.get('PF', 0)
            _pf_unreliable = _av.get('PF', 0) < 5.0 or _pf_total <= 0
            all_teams.append({
                'team': team,
                ...
                'ppg': _av['PTS'], 'oppg': _opp['PTS'],
                'stl': _av['STL'], 'blk': _av['BLK'],
                'ast': _av['AST'], 'tov': _av['TO'],
                'nst_pct': round(max(_to - _ostl, 0) / _poss * 100, 1),
                'twop': round((_av.get('FGM', 0) - _av.get('3PM', 0)) / max(_av.get('FGA', 0) - _av.get('3PA', 0), 1) * 100, 1) if (_av.get('FGA', 0) - _av.get('3PA', 0)) > 0 else 0,
                'tpp': round(_av.get('3PM', 0) / max(_av.get('3PA', 1), 1) * 100, 1) if _av.get('3PA', 0) > 0 else 0,
                'ftp': round(_av.get('FTM', 0) / max(_av.get('FTA', 1), 1) * 100, 1) if _av.get('FTA', 0) > 0 else 0,
                'tpa_pct': round(_av.get('3PA', 0) / max(_av.get('FGA', 1), 1) * 100, 1) if _av.get('FGA', 0) > 0 else 0,
                'ast_pct': round(_tot.get('AST', 0) / max(_tot.get('FGM', 1), 1) * 100, 1),
                'ast_ratio': round(100 * _av.get('AST', 0) / _ast_den, 1),
                'ast_tov': round(_av.get('AST', 0) / max(_av.get('TO', 0.01), 0.01), 2),
                'drebpg': _av.get('DREB', 0),
                'dreb_pct_adv': a.get('dreb_pct', 0),
                'spg': _av.get('STL', 0),
                'stl_pct_team': _stl_pct,
                'stl_to': round(_av.get('STL', 0) / max(_opp.get('TO', 0.01), 0.01), 2),
                'bpg': _av.get('BLK', 0),
                'blk_pct_team': _blk_pct,
                'hkm_pct': round(_blk_pct + _stl_pct, 1),
                'pf_total': None if _pf_unreliable else _pf_total,
                'pfpg': None if _pf_unreliable else _av.get('PF', 0),
                'pf_eff': None if _pf_unreliable else round((_tot.get('STL', 0) + _tot.get('BLK', 0)) / max(_pf_total, 0.1), 2),
                'stl_pf': None if _pf_unreliable else round(_tot.get('STL', 0) / max(_pf_total, 0.1), 2),
                'blk_pf': None if _pf_unreliable else round(_tot.get('BLK', 0) / max(_pf_total, 0.1), 2),
            })
        except: pass
...
def state_rank(key, val, higher_better=True):
    ...
    vals = sorted([t[key] for t in all_teams if t.get(key) not in (None, 0)], reverse=higher_better)
    rank = next((i+1 for i,v in enumerate(vals) if (higher_better and v<=val) or (not higher_better and v>=val)), len(vals))
    avg_pool = [t[key] for t in all_teams if t.get(key) is not None]
    avg = sum(avg_pool) / max(len(avg_pool), 1)
...
def team_adv_snapshot(av, opp, tot, otot, adv):
    poss = max(adv.get('possessions', 0.01), 0.01)
    ast_den = max(av.get('FGA', 0) + 0.475 * av.get('FTA', 0) + av.get('AST', 0) + av.get('TO', 0), 1)
    opp_poss_est = max((otot.get('FGA', 0) - otot.get('OREB', 0)) + otot.get('TO', 0) + 0.475 * otot.get('FTA', 0), 0.1)
    stl_pct_v = round(tot.get('STL', 0) / opp_poss_est * 100, 1)
    blk_pct_v = round(tot.get('BLK', 0) / max(otot.get('FGA', 1), 1) * 100, 1)
    pf_total_v = tot.get('PF', 0)
    return {
        'ortg': adv.get('ortg', 0),
        'ts_pct': adv.get('ts_pct', 0),
        'twop': round((av.get('FGM', 0) - av.get('3PM', 0)) / max(av.get('FGA', 0) - av.get('3PA', 0), 1) * 100, 1) if (av.get('FGA', 0) - av.get('3PA', 0)) > 0 else 0,
        'tpp': round(av.get('3PM', 0) / max(av.get('3PA', 1), 1) * 100, 1) if av.get('3PA', 0) > 0 else 0,
        'ftp': round(av.get('FTM', 0) / max(av.get('FTA', 1), 1) * 100, 1) if av.get('FTA', 0) > 0 else 0,
        'ast_pct': round(tot.get('AST', 0) / max(tot.get('FGM', 1), 1) * 100, 1),
        'ast_ratio': round(100 * av.get('AST', 0) / ast_den, 1),
        'tov_pct': adv.get('tov_pct', 0),
        'ast_tov': round(av.get('AST', 0) / max(av.get('TO', 0.01), 0.01), 2),
        'nst_pct': round(max(av.get('TO', 0) - opp.get('STL', 0), 0) / poss * 100, 1),
        'oreb_pct': adv.get('oreb_pct', 0),
        'ft_rate': adv.get('ft_rate', 0),
        'tpa_pct': round(av.get('3PA', 0) / max(av.get('FGA', 1), 1) * 100, 1) if av.get('FGA', 0) > 0 else 0,
        'pace': adv.get('possessions', 0),
        'drtg': adv.get('drtg', 0),
        'drebpg': av.get('DREB', 0),
        'dreb_pct': adv.get('dreb_pct', 0),
        'spg': av.get('STL', 0),
        'stl_pct_team': stl_pct_v,
        'stl_to': round(av.get('STL', 0) / max(opp.get('TO', 0.01), 0.01), 2),
        'bpg': av.get('BLK', 0),
        'blk_pct_team': blk_pct_v,
        'hkm_pct': round(stl_pct_v + blk_pct_v, 1),
        'pf_total': pf_total_v,
        'pfpg': av.get('PF', 0),
        'pf_eff': round((tot.get('STL', 0) + tot.get('BLK', 0)) / max(pf_total_v, 0.1), 2),
        'stl_pf': round(tot.get('STL', 0) / max(pf_total_v, 0.1), 2),
        'blk_pf': round(tot.get('BLK', 0) / max(pf_total_v, 0.1), 2),
    }

mp_adv_team = team_adv_snapshot(avgs, opp_avgs, totals, opp_totals, ta)
...
# ── Advanced analytics ranked pool ────────────────────────────────────────────
adv_players = []
    ...
    for _team in os.listdir(_cp):
        ...
        try:
            ...
            for _p in _ps['players']:
                ...
                _fga_pg = _tot.get('FGA', 0) / _gp
                _shooting_attempts_pg = (_tot.get('FGA', 0) + _tot.get('FTA', 0)) / _gp
                adv_players.append({
                    'name': _p['name'], 'team': _team,
                    'region': next((r['region'] for r in all_teams if r.get('team') == _team), ''),
                    'conference': _conf,
                    'gp': _tot.get('games', 0),
                    'min_pct': _min_pct,
                    ...
                    'fga_pg': _fga_pg,
                    'shooting_attempts_pg': _shooting_attempts_pg,
                })
        except: pass

if not adv_players:
    for _cache_path in [
        '/Users/chadoconnor/Neural Network - Codex Optimized/internal_data/cache/leaderboard/2526_players.json',
        f'{BASE}/internal_data/cache/leaderboard/2526_players.json',
    ]:
        if not os.path.exists(_cache_path):
            continue
        try:
            for _p in jload(_cache_path):
                _gp = max(_p.get('gp', 0), 1)
                adv_players.append({
                    'name': _p.get('name', ''),
                    'team': _p.get('school', ''),
                    'region': _p.get('region', ''),
                    'conference': _p.get('conference', ''),
                    'gp': _p.get('gp', 0),
                    'min_pct': _p.get('min_pct', 0),
                    'ind_ortg': _p.get('ind_ortg', 0),
                    'ind_drtg': _p.get('ind_drtg', 0),
                    'efg_pct': _p.get('efg_pct', 0),
                    'ts_pct': _p.get('ts_pct', 0),
                    'ast_rate': _p.get('ast_rate', 0),
                    'tov_pct': _p.get('tov_pct', 0),
                    'oreb_pct': _p.get('oreb_pct', 0),
                    'dreb_pct': _p.get('dreb_pct', 0),
                    'stl_pct': _p.get('stl_pct', 0),
                    'blk_pct': _p.get('blk_pct', 0),
                    'usage_pct': _p.get('usage_pct', 0),
                    'shot_pct': _p.get('shot_pct', 0),
                    'ft_rate': _p.get('ft_rate', 0),
                    'fc_per_40': _p.get('fc_per_40', 0),
                    'fd_per_40': _p.get('fd_per_40', 0),
                    'twop': _p.get('twop', 0),
                    'twoa_pg': _p.get('twoa', 0) / _gp,
                    'tpp': _p.get('tpp', 0),
                    'tpa_pg': _p.get('tpa', 0) / _gp,
                    'ftp': _p.get('ftp', 0),
                    'fta_pg': _p.get('fta', 0) / _gp,
                    'fga_pg': _p.get('fga', 0) / _gp,
                    'shooting_attempts_pg': (_p.get('fga', 0) + _p.get('fta', 0)) / _gp,
                })
            break
        except Exception:
            adv_players = []
...
def adv_rank(key, val, hb=True):
    val = _num(val)
    vals = sorted([_num(p.get(key)) for p in adv_players if _num(p.get(key)) > 0], reverse=hb)
    r = next((i+1 for i,v in enumerate(vals) if (hb and v<=val) or (not hb and v>=val)), len(vals))
    return r, len(vals)
...
ADV_RELAXED_CAT_KEYS = {'efg_pct', 'ts_pct', 'ft_rate', 'twop', 'tpp', 'ftp'}
_adv_relaxed_cache = {}

def adv_cat_standard_eligible(p, key):
    if _num(p.get(key)) <= 0:
        return False
    if key in {'efg_pct', 'ts_pct', 'ft_rate'} and _num(p.get('min_pct')) < 50:
        return False
    elif key not in {'efg_pct', 'ts_pct', 'ft_rate'} and _num(p.get('min_pct')) < 30:
        return False
    if key == 'usage_pct' and _num(p.get('usage_pct')) > 40:
        return False
    if key == 'twop' and _num(p.get('twoa_pg')) < 2.0:
        return False
    if key == 'tpp' and _num(p.get('tpa_pg')) < 2.0:
        return False
    if key == 'ftp' and _num(p.get('fta_pg')) < 2.0:
        return False
    return True

def school_needs_relaxed_cat(team, key):
    if key not in ADV_RELAXED_CAT_KEYS:
        return False
    cache_key = (team, key)
    if cache_key not in _adv_relaxed_cache:
        school_rows = [p for p in adv_players if p.get('team') == team]
        _adv_relaxed_cache[cache_key] = not any(adv_cat_standard_eligible(p, key) for p in school_rows)
    return _adv_relaxed_cache[cache_key]

def adv_cat_relaxed_eligible(p, key):
    if _num(p.get(key)) <= 0 or not school_needs_relaxed_cat(p.get('team'), key):
        return False
    if _num(p.get('gp')) < 20:
        return False
    if key == 'efg_pct':
        return _num(p.get('fga_pg')) >= 2.5
    if key == 'ts_pct':
        return _num(p.get('shooting_attempts_pg')) >= 2.5
    if key == 'ft_rate':
        return True
    if key == 'twop':
        return _num(p.get('twoa_pg')) >= 2.5
    if key == 'tpp':
        return _num(p.get('tpa_pg')) >= 2.5
    if key == 'ftp':
        return _num(p.get('fta_pg')) >= 2.5
    return False

def adv_cat_eligible(p, key):
    return adv_cat_standard_eligible(p, key) or adv_cat_relaxed_eligible(p, key)

def adv_rank_qualified(key, val, hb=True):
    val = _num(val)
    pool = [p for p in adv_players if adv_cat_eligible(p, key) and _num(p.get(key)) > 0]
    vals = sorted([_num(p.get(key)) for p in pool], reverse=hb)
    if not vals:
        return 0, 0
    r = next((i+1 for i,v in enumerate(vals) if (hb and v <= val) or (not hb and v >= val)), len(vals) + 1)
    return r, len(vals)
...
def player_top100_adv_flags(adv, avg, tot):
    ...
    if min_pct < 30:
        return []
    fga_pg = fga / gp
    shooting_attempts_pg = (fga + tot.get('FTA', 0)) / gp
    values = {
        'team': 'Moorpark',
        'gp': tot.get('games', avg.get('games', 0)),
        ...
        'fga_pg': fga_pg,
        'shooting_attempts_pg': shooting_attempts_pg,
    }
    flags = []
...
def top100_adv_sentence(name, key, label, value, rank, total):
    ...
    if key == 'twop':
        return f'Inside the arc, his <b>{value} 2P%</b> ranks <b>#{rank}</b> in the qualified 2P% pool.'
    if key == 'tpp':
        return f'From three, his <b>{value} 3P%</b> ranks <b>#{rank}</b> in the qualified 3P% pool.'
    if key == 'ftp':
        return f'At the line, his <b>{value} FT%</b> ranks <b>#{rank}</b> in the qualified FT% pool.'
    return f'He ranks <b>#{rank}</b> in {label} at <b>{value}</b>.'

def metric_num(value):
    try:
        return float(str(value).replace('%', '').strip())
    except Exception:
        return 0.0

def stat_quality(key, value):
    v = metric_num(value)
    if key == 'ind_ortg':
        if v >= 120:
            return 'an excellent individual-efficiency level'
        if v >= 110:
            return 'a good individual-efficiency level'
    if key == 'oreb_pct' and v >= 10:
        return 'a strong offensive-rebounding mark'
    if key == 'dreb_pct' and v >= 20:
        return 'a strong defensive-rebounding mark'
    if key == 'ft_rate':
        if v >= 70:
            return 'an excellent free-throw-rate mark'
        if v >= 50:
            return 'a good free-throw-rate mark'
    if key == 'tov_pct' and v < 15:
        return 'a great turnover-care mark'
    if key == 'blk_pct' and v > 8:
        return 'a very good block-rate mark'
    if key == 'stl_pct' and v > 5:
        return 'a very good steal-rate mark'
    return ''

def with_quality(key, value):
    q = stat_quality(key, value)
    return f' ({q})' if q else ''

def top100_adv_blended_sentence(key, label, value, rank, total):
    if key == 'ind_ortg':
        return f'That offensive profile is backed by a <b>{value} ORtg</b>{with_quality(key, value)}, ranked <b>#{rank}</b> among {total} qualified players.'
    if key == 'ind_drtg':
        return f'The defensive box-score layer supports the eye test too: <b>{value} DRtg</b>, <b>#{rank}</b> among {total} qualified players.'
    if key == 'efg_pct':
        return f'The shot profile carries real value, with a <b>{value} eFG%</b> that ranks <b>#{rank}</b> in the qualified eFG% pool.'
    if key == 'ts_pct':
        return f'His scoring efficiency holds up after free throws as well: <b>{value} TS%</b>, ranked <b>#{rank}</b> in the qualified TS% pool.'
    if key == 'min_pct':
        return f'The workload is not cosmetic either; he plays <b>{value} of team minutes</b>, ranking <b>#{rank}</b> among qualified players.'
    if key == 'usage_pct':
        return f'That role is a genuine possession load, with <b>{value} %Poss</b> ranking <b>#{rank}</b> among players under the 40% usage cap.'
    if key == 'shot_pct':
        return f'Moorpark lets him end possessions at a top-100 rate: <b>{value} %Shots</b>, ranked <b>#{rank}</b> among qualified players.'
    if key == 'oreb_pct':
        return f'The offensive glass is part of the pressure: <b>{value} OREB%</b>{with_quality(key, value)}, ranked <b>#{rank}</b> statewide.'
    if key == 'dreb_pct':
        return f'He also finishes defensive possessions on the glass, with a <b>{value} DREB%</b>{with_quality(key, value)} ranked <b>#{rank}</b>.'
    if key == 'tov_pct':
        return f'The ball-security number is a real separator: <b>{value} TO%</b>{with_quality(key, value)}, ranked <b>#{rank}</b> among qualified players.'
    if key == 'ast_rate':
        return f'The creation translates statewide, with a <b>{value} assist rate</b> ranked <b>#{rank}</b>.'
    if key == 'blk_pct':
        return f'The rim-protection signal is measurable too: <b>{value} block rate</b>{with_quality(key, value)}, ranked <b>#{rank}</b>.'
    if key == 'stl_pct':
        return f'That activity shows up in steals, where his <b>{value} steal rate</b>{with_quality(key, value)} ranks <b>#{rank}</b>.'
    if key == 'ft_rate':
        return f'He turns that pressure into free throws at a top-100 level, with a <b>{value} FT Rate</b>{with_quality(key, value)} ranked <b>#{rank}</b>.'
    if key == 'fc_per_40':
        return f'He also avoids cheap fouls, committing only <b>{value} fouls per 40</b>, ranked <b>#{rank}</b>.'
    if key == 'fd_per_40':
        return f'He draws contact at a top-100 rate, with <b>{value} fouls drawn per 40</b> ranked <b>#{rank}</b>.'
    if key == 'twop':
        return f'Inside the arc, the efficiency is top-100 level: <b>{value} 2P%</b>, ranked <b>#{rank}</b> in the qualified 2P% pool.'
    if key == 'tpp':
        return f'The perimeter shooting is leaderboard-backed: <b>{value} 3P%</b>, ranked <b>#{rank}</b> in the qualified 3P% pool.'
    if key == 'ftp':
        return f'The free-throw piece is also reliable, with <b>{value} FT%</b> ranked <b>#{rank}</b> in the qualified FT% pool.'
    return f'His <b>{label}</b> is top-100 statewide at <b>{value}</b>, ranked <b>#{rank}</b>.'

def insert_after_theme_sentence(narrative, anchors, insert_text):
    lower = narrative.lower()
    for anchor in anchors:
        idx = lower.find(anchor)
        if idx == -1:
            continue
        end = narrative.find('. ', idx)
        if end == -1:
            return f'{narrative} {insert_text}'
        return f'{narrative[:end+2]}{insert_text} {narrative[end+2:]}'
    end = narrative.rfind('. ')
    if end == -1:
        return f'{narrative} {insert_text}'
    return f'{narrative[:end+2]}{insert_text} {narrative[end+2:]}'

def blend_top100_into_narrative(narrative, flags):
    theme_defs = [
        ({'ast_rate'}, ['playmaker', 'assist', 'sets everyone', 'engine', 'passing']),
        ({'tov_pct'}, ['turnover', 'give the ball away', 'ball security', 'tov']),
        ({'efg_pct', 'ts_pct', 'twop', 'tpp', 'ftp', 'ft_rate', 'shot_pct'}, ['catch-and-shoot', 'three-point', '3-point', 'perimeter', 'efficiency', 'shoot', 'line', 'rim', 'scor']),
        ({'min_pct', 'usage_pct'}, ['minutes', 'floor time', 'role', 'usage', 'possessions']),
        ({'ind_drtg', 'oreb_pct', 'dreb_pct', 'blk_pct', 'stl_pct', 'fc_per_40', 'fd_per_40'}, ['defens', 'rebound', 'steal', 'block', 'foul']),
        ({'ind_ortg'}, ['offense', 'efficiency', 'net rating', 'scor']),
    ]
    remaining = list(flags)
    for keys, anchors in theme_defs:
        themed = [flag for flag in remaining if flag[1] in keys]
        if not themed:
            continue
        themed.sort(key=lambda x: x[0])
        insert_text = ' '.join(
            top100_adv_blended_sentence(key, label, value, rank, total)
            for rank, key, label, value, total in themed
        )
        narrative = insert_after_theme_sentence(narrative, anchors, insert_text)
        remaining = [flag for flag in remaining if flag[1] not in keys]
    if remaining:
        insert_text = ' '.join(
            top100_adv_blended_sentence(key, label, value, rank, total)
            for rank, key, label, value, total in remaining
        )
        narrative = insert_after_theme_sentence(narrative, ['profile', 'role', 'scor'], insert_text)
    return narrative
...
# ── Player role from %Poss and Min% ────────────────────────────────────────────
def player_role_from_poss(usage_pct: float, mpg: float) -> tuple[str, str]:
    """Return (role_label, hex_color) based on local %Poss tiers.
...
# Merge player map
def unweighted_game_rating(name):
    match_key = next((k for k in plyr_rtgs.get('game_logs', {}) if name in k), None)
    games = plyr_rtgs.get('game_logs', {}).get(match_key, []) if match_key else []
    if not games:
        return {}
    ortgs = [g.get('ortg', 0) for g in games]
    drtgs = [g.get('drtg', 0) for g in games]
    nets = [g.get('net', g.get('ortg', 0) - g.get('drtg', 0)) for g in games]
    mp = sum(g.get('MP', 0) for g in games)
    return {
        'name': match_key or name,
        'games': len(games),
        'MP': mp,
        'mpg': round(mp / len(games), 1),
        'ortg': round(sum(ortgs) / len(ortgs), 1),
        'drtg': round(sum(drtgs) / len(drtgs), 1),
        'net': round(sum(nets) / len(nets), 1),
    }
...
for p in plyr_stats['players']:
    player_map[p['name']] = {
        ...
        'rating': unweighted_game_rating(p['name']),
    }
...
def page_cover():
    ...
    twop_r,  _, twop_avg,  _ = state_rank('twop',        mp_adv_team['twop'], True)
    tpp_r,   _, tpp_avg,   _ = state_rank('tpp',         mp_adv_team['tpp'], True)
    ftp_r,   _, ftp_avg,   _ = state_rank('ftp',         mp_adv_team['ftp'], True)
    astp_r,  _, astp_avg,  _ = state_rank('ast_pct',     mp_adv_team['ast_pct'], True)
    astr_r,  _, astr_avg,  _ = state_rank('ast_ratio',   mp_adv_team['ast_ratio'], True)
    astt_r,  _, astt_avg,  _ = state_rank('ast_tov',     mp_adv_team['ast_tov'], True)
    tpar_r,  _, tpar_avg,  _ = state_rank('tpa_pct',     mp_adv_team['tpa_pct'], True)
    stlp_r,  _, stlp_avg,  _ = state_rank('stl_pct_team', mp_adv_team['stl_pct_team'], True)
    stlto_r, _, stlto_avg, _ = state_rank('stl_to',      mp_adv_team['stl_to'], True)
    bpg_r,   _, bpg_avg,   _ = state_rank('bpg',         mp_adv_team['bpg'], True)
    blkp_r,  _, blkp_avg,  _ = state_rank('blk_pct_team', mp_adv_team['blk_pct_team'], True)
    hkm_r,   _, hkm_avg,   _ = state_rank('hkm_pct',     mp_adv_team['hkm_pct'], True)
    pfpg_r,  _, pfpg_avg,  _ = state_rank('pfpg',        mp_adv_team['pfpg'], False)
    pfe_r,   _, pfe_avg,   _ = state_rank('pf_eff',      mp_adv_team['pf_eff'], True)
    stlpf_r, _, stlpf_avg, _ = state_rank('stl_pf',      mp_adv_team['stl_pf'], True)
    blkpf_r, _, blkpf_avg, _ = state_rank('blk_pf',      mp_adv_team['blk_pf'], True)

    e.append(Paragraph(
        '<b>Identity: they are a defense-first team with a high-variance offensive engine.</b> '
        f'The cleanest read on Moorpark is the split between a top-tier defense and an ordinary offense: '
        f'<b>{ta["drtg"]:.1f} DRTg</b> ranks <b>#{drtg_r}/100</b>, while <b>{ta["ortg"]:.1f} ORtg</b> sits '
        f'<b>#{ortg_r}/100</b>. The net result is still good (<b>+{ta["net_rtg"]:.1f}</b>, #{net_rank}/100), '
        'but the way they get there matters. They are not a balanced, inevitable scoring machine. '
        'They win by defending well enough to let a three-heavy, second-chance offense survive its cold stretches.',
        S(9.5, space_after=7)))

    e.append(Paragraph(
        '<b>Adv. Offense: threes, assists, rebounds, and a glaring lack of foul pressure.</b> '
        f'Moorpark\'s shot diet is extreme: <b>{mp_adv_team["tpa_pct"]:.1f}% 3PAr</b>, ranked '
        f'<b>#{tpar_r}/100</b>, with only middling interior finishing (<b>{mp_adv_team["twop"]:.1f}% 2P</b>, '
        f'#{twop_r}) and solid but not elite perimeter accuracy (<b>{mp_adv_team["tpp"]:.1f}% 3P</b>, #{tpp_r}). '
        f'Their overall scoring profile lands near the middle nationally within the CCCAA pool: '
        f'<b>{ta["ts_pct"]:.1f}% TS</b> (#{ts_r}) and <b>{ta["efg_pct"]:.1f}% eFG</b> (#{efg_r}). '
        'The offense looks best when the ball is moving into rhythm threes: '
        f'<b>{mp_adv_team["ast_pct"]:.1f}% AST%</b> ranks #{astp_r}, <b>{mp_adv_team["ast_ratio"]:.1f} AST Ratio</b> ranks #{astr_r}, '
        f'and <b>{mp_adv_team["ast_tov"]:.2f} AST/TOV</b> ranks #{astt_r}. This is a connected offense, not a downhill one.',
        S(9.5, space_after=7)))

    e.append(Paragraph(
        '<b>The pressure point is obvious: make them score without threes or putbacks.</b> '
        f'They compensate for shot variance by hitting the glass: <b>{ta["oreb_pct"]:.1f}% ORB%</b> ranks '
        f'<b>#{oreb_r}/100</b>. That is not a side note; it is part of the offensive system. '
        f'But the foul-line profile is a hard ceiling. Their <b>{ta["ft_rate"]:.1f} FTA Rate</b> ranks '
        f'<b>#{ftr_r}/100</b>, one of the weakest marks in the state, despite a decent <b>{mp_adv_team["ftp"]:.1f}% FT%</b> '
        f'(#{ftp_r}) when they actually get there. They also carry a hidden turnover issue: '
        f'<b>{_mp_nst:.1f}% NST%</b> ranks <b>#{nst_r}/100</b>, meaning too many mistakes are self-created rather '
        'than forced by steals. The game plan is straightforward: stay attached to shooters, finish possessions '
        'with box-outs, and do not bail them out with fouls.',
        S(9.5, space_after=8)))

    e.append(Paragraph(
        '<b>Adv. Defense: the rating is better than the havoc profile.</b> '
        f'Moorpark\'s defensive efficiency is excellent because opponents do not shoot comfortably against them: '
        f'<b>{ta["opp_efg_pct"]:.1f}% Opp eFG</b> ranks <b>#{oefg_r}/100</b>, and <b>{ta["opp_ts_pct"]:.1f}% Opp TS</b> '
        f'ranks <b>#{ots_r}/100</b>. That is the backbone. The disruption numbers are less dominant than the rating: '
        f'<b>{mp_adv_team["stl_pct_team"]:.1f}% STL%</b> ranks #{stlp_r}, <b>{mp_adv_team["stl_to"]:.2f} STL/TOV</b> '
        f'ranks #{stlto_r}, and <b>{mp_adv_team["hkm_pct"]:.1f}% HKM%</b> ranks #{hkm_r}. They are not winning by '
        'overwhelming teams with steals and blocks. They are winning by making shots uncomfortable and keeping the game inside their defensive rules.',
        S(9.5, space_after=7)))

    e.append(Paragraph(
        '<b>The defensive weaknesses are discipline and the glass, not scheme.</b> '
        f'Their defensive rebounding is ordinary: <b>{ta["dreb_pct"]:.1f}% DRB%</b> ranks #{dreb_r}, with '
        f'<b>{mp_adv_team["drebpg"]:.1f} DRB/G</b> around the state middle. The block profile is also only moderate '
        f'(<b>{mp_adv_team["bpg"]:.1f} BLK</b>, #{bpg_r}; <b>{mp_adv_team["blk_pct_team"]:.1f}% BLK%</b>, #{blkp_r}). '
        f'Fouls are the bigger concern: <b>{mp_adv_team["pfpg"]:.1f} PF/G</b> ranks #{pfpg_r}, while '
        f'<b>{mp_adv_team["pf_eff"]:.2f} PF Eff</b> ranks #{pfe_r}, <b>{mp_adv_team["stl_pf"]:.2f} STL/PF</b> ranks #{stlpf_r}, '
        f'and <b>{mp_adv_team["blk_pf"]:.2f} BLK/PF</b> ranks #{blkpf_r}. In plain terms, they foul more than their '
        'event creation justifies. Attack with composure, force rotations, and make their defenders guard without reaching.',
        S(9.5, space_after=8)))

    e.append(Paragraph(
        '<b>Tempo: fast enough to punish mistakes, not fast enough to cover every flaw.</b> '
        f'Moorpark plays at <b>{ta["possessions"]:.1f} possessions per game</b> (#{pace_r}/100). '
        'That pace amplifies runs: live-ball stops become early threes, and offensive rebounds extend pressure. '
        'But if an opponent slows the game, contests the arc, and cleans the defensive glass, Moorpark has to win '
        'through half-court shot-making and disciplined decision-making. That is the version of the game opponents should try to create.',
        S(9.5, space_after=8)))

    e.append(Paragraph(
        '<b>Scouting thesis:</b> Moorpark is dangerous because its defense gives the offense extra chances to be streaky. '
        'Do not treat them like a bad offensive team; treat them like a volatile one. The shot profile can bury you quickly, '
        'but the counters are clear: run them off the line, gang rebound, value the ball against their pressure, and put their '
        'defense into foul decisions.',
        S(9.5, space_after=8)))
...
def page_analytics():
    ...
    e += section('Adv. Offense and Adv. Defense',
                 'Leaderboard context translated into scouting language.')

    def required_row(label, key, val, hb, unit='', note='', fmt='{:.1f}'):
        r, tot, avg, diff = state_rank(key, val, hb)
        sign = '+' if diff > 0 else ''
        return [
            Paragraph(label, S(7.5, True)),
            fmt.format(val) + unit,
            f'#{r}/{tot}',
            f'{avg:.1f}{unit}',
            f'{sign}{diff:.1f}{unit}',
            Paragraph(note, S(7.2)),
        ]

    req_h = ['METRIC', 'MPC', 'RANK', 'AVG', 'VS AVG', 'SCOUTING READ']
    off_rows = [
        required_row('ORtg', 'ortg', mp_adv_team['ortg'], True, '', 'Baseline scoring efficiency per 100 possessions.'),
        required_row('TS%', 'ts_pct', mp_adv_team['ts_pct'], True, '%', 'Overall scoring efficiency including threes and free throws.'),
        required_row('2P%', 'twop', mp_adv_team['twop'], True, '%', 'Interior/two-point finishing quality; shows whether there is a non-3P counter.'),
        required_row('3P%', 'tpp', mp_adv_team['tpp'], True, '%', 'Perimeter accuracy; must be read with 3PAr.'),
        required_row('FT%', 'ftp', mp_adv_team['ftp'], True, '%', 'Conversion when they do reach the line.'),
        required_row('AST%', 'ast_pct', mp_adv_team['ast_pct'], True, '%', 'Share of made shots created by assists.'),
        required_row('AST Ratio', 'ast_ratio', mp_adv_team['ast_ratio'], True, '', 'Assist creation per offensive action; quality of ball movement.'),
        required_row('TOV%', 'tov_pct', mp_adv_team['tov_pct'], False, '%', 'Possession loss rate; lower is better.'),
        required_row('AST/TOV', 'ast_tov', mp_adv_team['ast_tov'], True, '', 'Creation-to-mistake balance.', '{:.2f}'),
        required_row('NST%', 'nst_pct', mp_adv_team['nst_pct'], False, '%', 'Unforced turnover profile: turnovers not explained by opponent steals.'),
        required_row('ORB%', 'oreb_pct', mp_adv_team['oreb_pct'], True, '%', 'Second-chance creation; especially important for jump-shooting teams.'),
        required_row('FTA Rate', 'ft_rate', mp_adv_team['ft_rate'], True, '', 'Contact pressure and free-point generation.'),
        required_row('3PAr', 'tpa_pct', mp_adv_team['tpa_pct'], True, '%', 'Shot diet: how much of the offense is built on threes.'),
        required_row('Pace', 'pace', mp_adv_team['pace'], True, '', 'Tempo and transition/early-offense pressure.'),
    ]
    def req_table(title, rows):
        e.append(Paragraph(f'<b>{title}</b>', S(8.8, True, BLUE, space_after=3)))
        t = Table([req_h] + rows, colWidths=[1.05*inch, 0.55*inch, 0.55*inch, 0.55*inch, 0.6*inch, 4.2*inch])
        t.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,0), TH_BG),
            ('TEXTCOLOR', (0,0), (-1,0), WHITE),
            ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'),
    ...
    req_table('Adv. Offense', off_rows)

    def_rows = [
        required_row('DRtg', 'drtg', mp_adv_team['drtg'], False, '', 'Baseline defensive efficiency per 100 possessions; lower is better.'),
        required_row('DRB', 'drebpg', mp_adv_team['drebpg'], True, '', 'Raw defensive boards per game.'),
        required_row('DRB%', 'dreb_pct', mp_adv_team['dreb_pct'], True, '%', 'How often the defense finishes opponent misses.'),
        required_row('STL', 'spg', mp_adv_team['spg'], True, '', 'Raw steal volume per game.'),
        required_row('STL%', 'stl_pct_team', mp_adv_team['stl_pct_team'], True, '%', 'Possession-level steal pressure.'),
        required_row('STL/TOV', 'stl_to', mp_adv_team['stl_to'], True, '', 'How much opponent turnover creation comes through steals.', '{:.2f}'),
        required_row('BLK', 'bpg', mp_adv_team['bpg'], True, '', 'Raw block volume per game.'),
        required_row('BLK%', 'blk_pct_team', mp_adv_team['blk_pct_team'], True, '%', 'Shot-blocking impact relative to opponent attempts.'),
        required_row('HKM%', 'hkm_pct', mp_adv_team['hkm_pct'], True, '%', 'Havoc marker combining steal and block pressure.'),
        required_row('PF', 'pf_total', mp_adv_team['pf_total'], False, '', 'Total foul load; contextual discipline marker.'),
        required_row('PF/G', 'pfpg', mp_adv_team['pfpg'], False, '', 'Foul rate per game; lower is cleaner.'),
        required_row('PF Eff', 'pf_eff', mp_adv_team['pf_eff'], True, '', 'Defensive events generated per foul.', '{:.2f}'),
        required_row('STL/PF', 'stl_pf', mp_adv_team['stl_pf'], True, '', 'Steals generated per foul committed.', '{:.2f}'),
        required_row('BLK/PF', 'blk_pf', mp_adv_team['blk_pf'], True, '', 'Blocks generated per foul committed.', '{:.2f}'),
    ]
    req_table('Adv. Defense', def_rows)
...
def page_analytics():
    e = []
    e += section('Adv. Offense and Adv. Defense',
                 'A scouting read built from the full advanced team leaderboard.')

    def rp(key, val, hb=True):
        r, total, avg, diff = state_rank(key, val, hb)
        return r, total, avg, diff

    ortg_r, _, ortg_avg, _ = rp('ortg', mp_adv_team['ortg'], True)
    drtg_r, _, drtg_avg, _ = rp('drtg', mp_adv_team['drtg'], False)
    ts_r, _, ts_avg, _ = rp('ts_pct', mp_adv_team['ts_pct'], True)
    twop_r, _, twop_avg, _ = rp('twop', mp_adv_team['twop'], True)
    tpp_r, _, tpp_avg, _ = rp('tpp', mp_adv_team['tpp'], True)
    ftp_r, _, ftp_avg, _ = rp('ftp', mp_adv_team['ftp'], True)
    astp_r, _, astp_avg, _ = rp('ast_pct', mp_adv_team['ast_pct'], True)
    astr_r, _, astr_avg, _ = rp('ast_ratio', mp_adv_team['ast_ratio'], True)
    tov_r, _, tov_avg, _ = rp('tov_pct', mp_adv_team['tov_pct'], False)
    astt_r, _, astt_avg, _ = rp('ast_tov', mp_adv_team['ast_tov'], True)
    nst_r, _, nst_avg, _ = rp('nst_pct', mp_adv_team['nst_pct'], False)
    orb_r, _, orb_avg, _ = rp('oreb_pct', mp_adv_team['oreb_pct'], True)
    ftr_r, _, ftr_avg, _ = rp('ft_rate', mp_adv_team['ft_rate'], True)
    tpar_r, _, tpar_avg, _ = rp('tpa_pct', mp_adv_team['tpa_pct'], True)
    pace_r, _, pace_avg, _ = rp('pace', mp_adv_team['pace'], True)
    dreb_r, _, dreb_avg, _ = rp('dreb_pct', mp_adv_team['dreb_pct'], True)
    drebpg_r, _, drebpg_avg, _ = rp('drebpg', mp_adv_team['drebpg'], True)
    spg_r, _, spg_avg, _ = rp('spg', mp_adv_team['spg'], True)
    stlp_r, _, stlp_avg, _ = rp('stl_pct_team', mp_adv_team['stl_pct_team'], True)
    stlto_r, _, stlto_avg, _ = rp('stl_to', mp_adv_team['stl_to'], True)
    bpg_r, _, bpg_avg, _ = rp('bpg', mp_adv_team['bpg'], True)
    blkp_r, _, blkp_avg, _ = rp('blk_pct_team', mp_adv_team['blk_pct_team'], True)
    hkm_r, _, hkm_avg, _ = rp('hkm_pct', mp_adv_team['hkm_pct'], True)
    pf_r, _, pf_avg, _ = rp('pf_total', mp_adv_team['pf_total'], False)
    pfpg_r, _, pfpg_avg, _ = rp('pfpg', mp_adv_team['pfpg'], False)
    pfe_r, _, pfe_avg, _ = rp('pf_eff', mp_adv_team['pf_eff'], True)
    stlpf_r, _, stlpf_avg, _ = rp('stl_pf', mp_adv_team['stl_pf'], True)
    blkpf_r, _, blkpf_avg, _ = rp('blk_pf', mp_adv_team['blk_pf'], True)

    e.append(Paragraph(
        '<b>Executive read:</b> Moorpark is a top-defense, average-offense profile with very specific levers. '
        f'The defense carries the team: <b>{mp_adv_team["drtg"]:.1f} DRTg</b> ranks <b>#{drtg_r}/100</b>. '
        f'The offense is much closer to the state middle: <b>{mp_adv_team["ortg"]:.1f} ORtg</b> ranks '
        f'<b>#{ortg_r}/100</b>, with <b>{mp_adv_team["ts_pct"]:.1f}% TS</b> ranking #{ts_r}. '
        'The scouting question is not whether Moorpark can score. They can. The question is whether you let them score '
        'in the specific ways their roster is built to score: rhythm threes, assisted jumpers, early offense, and second chances.',
        S(9.5, space_after=8)))

    e.append(Paragraph('<b>Adv. Offense: what they want the game to become</b>', S(9.5, True, BLUE, space_after=3)))
    e.append(Paragraph(
        f'This is a perimeter-volume offense. The clearest number is <b>{mp_adv_team["tpa_pct"]:.1f}% 3PAr</b>, '
        f'<b>#{tpar_r}/100</b> and almost ten points above the state average ({tpar_avg:.1f}%). '
        f'The accuracy is good enough to punish help mistakes (<b>{mp_adv_team["tpp"]:.1f}% 3P</b>, #{tpp_r}), '
        f'but the inside-the-arc profile is ordinary (<b>{mp_adv_team["twop"]:.1f}% 2P</b>, #{twop_r}). '
        f'That matters because their <b>{mp_adv_team["ft_rate"]:.1f} FTA Rate</b> ranks <b>#{ftr_r}/100</b>. '
        'They do not consistently turn drives into fouls, so the offense needs clean threes and extra possessions to keep pressure on the scoreboard.',
        S(9.5, space_after=6)))
    e.append(Paragraph(
        f'The better part of the offense is its connectivity. Moorpark posts <b>{mp_adv_team["ast_pct"]:.1f}% AST%</b> '
        f'(#{astp_r}), <b>{mp_adv_team["ast_ratio"]:.1f} AST Ratio</b> (#{astr_r}), and '
        f'<b>{mp_adv_team["ast_tov"]:.2f} AST/TOV</b> (#{astt_r}). That is the evidence behind the film read: '
        'their good shots usually come from a pass, reversal, drive-and-kick, or early catch, not from isolated creators beating a set defense. '
        f'The problem is the mistake profile. Their <b>{mp_adv_team["tov_pct"]:.1f}% TOV%</b> is ordinary (#{tov_r}), '
        f'but <b>{mp_adv_team["nst_pct"]:.1f}% NST%</b> ranks <b>#{nst_r}/100</b>, which points to self-inflicted errors. '
        'They can look organized and careless in the same half.',
        S(9.5, space_after=6)))
    e.append(Paragraph(
        f'The offensive glass is the pressure valve. Their <b>{mp_adv_team["oreb_pct"]:.1f}% ORB%</b> ranks '
        f'<b>#{orb_r}/100</b>, and it changes how you defend them. Contesting the first three is not enough if the miss '
        'turns into another kick-out. The best defensive possession against Moorpark ends with a rebound, not merely a contest.',
        S(9.5, space_after=8)))

    e.append(Paragraph('<b>Adv. Defense: what the rating hides and reveals</b>', S(9.5, True, BLUE, space_after=3)))
    e.append(Paragraph(
        f'The headline is real: <b>{mp_adv_team["drtg"]:.1f} DRTg</b>, <b>#{drtg_r}/100</b>. '
        'But the path to that rating is more about shot quality than chaos. '
        f'Moorpark is not elite in raw disruption: <b>{mp_adv_team["spg"]:.1f} steals</b> rank #{spg_r}, '
        f'<b>{mp_adv_team["stl_pct_team"]:.1f}% STL%</b> ranks #{stlp_r}, and '
        f'<b>{mp_adv_team["stl_to"]:.2f} STL/TOV</b> ranks #{stlto_r}. '
        f'The combined havoc marker, <b>{mp_adv_team["hkm_pct"]:.1f}% HKM%</b>, ranks #{hkm_r}. '
        'So this is not a defense that simply takes the ball from you every trip. It is a defense that makes possessions feel smaller: fewer clean windows, fewer comfortable catches, and more late-clock decisions.',
        S(9.5, space_after=6)))
    e.append(Paragraph(
        f'The rim and glass numbers are functional, not dominant. Moorpark blocks <b>{mp_adv_team["bpg"]:.1f}</b> shots per game '
        f'(#{bpg_r}) with a <b>{mp_adv_team["blk_pct_team"]:.1f}% BLK%</b> (#{blkp_r}), while the defensive glass sits near the middle: '
        f'<b>{mp_adv_team["drebpg"]:.1f} DRB</b> (#{drebpg_r}) and <b>{mp_adv_team["dreb_pct"]:.1f}% DRB%</b> (#{dreb_r}). '
        'That creates a clear opponent priority: do not settle for the first contested jumper, and send enough rebounding pressure to test whether Moorpark can finish the stop.',
        S(9.5, space_after=6)))
    e.append(Paragraph(
        f'The foul profile is where the defense can be attacked. Moorpark has <b>{mp_adv_team["pf_total"]:.0f}</b> total fouls '
        f'(#{pf_r}) and <b>{mp_adv_team["pfpg"]:.1f} PF/G</b> (#{pfpg_r}); lower is better in both. '
        f'The event-per-foul ratios are not strong enough to fully justify the whistle burden: '
        f'<b>{mp_adv_team["pf_eff"]:.2f} PF Eff</b> ranks #{pfe_r}, <b>{mp_adv_team["stl_pf"]:.2f} STL/PF</b> ranks #{stlpf_r}, '
        f'and <b>{mp_adv_team["blk_pf"]:.2f} BLK/PF</b> ranks #{blkpf_r}. The right opponent should not avoid contact. '
        'Make them defend the paint, make them tag cutters, and make their pressure become fouls instead of deflections.',
        S(9.5, space_after=8)))

    e.append(Paragraph('<b>How this should change the game plan</b>', S(9.5, True, BLUE, space_after=3)))
    e.append(Paragraph(
        'Against Moorpark, the defensive plan should start with the math: reduce 3PA volume, finish the possession on the glass, '
        'and live with contested twos. On offense, the plan is patience with purpose: avoid loose-ball turnovers, make the defense guard '
        'multiple actions, and attack the foul profile. Moorpark is good enough defensively to punish sloppy teams and explosive enough '
        'from three to flip a game quickly. They are also specific enough that a disciplined opponent can target the weak points.',
        S(9.5, space_after=8)))
...
def page_rotation():
    ...
    e += section('Player Rotation & Impact Ratings',
                 'Individual ratings are unweighted averages of each player\'s Dean Oliver game ratings. '
                 'Zero-rating games are included; no minute weighting or trimming is applied.')

    rot_h = ['#', 'PLAYER', 'POS', 'GP', 'MPG', 'AVG ORTg', 'AVG DRTg', 'AVG NET', 'INTERP.']
    rot_r, rc = [], []
    ...
    e.append(Spacer(1, 6))
    e.append(Paragraph(
        '<b>Reading individual ratings:</b> AVG ORtg / AVG DRtg are simple averages of each player\'s '
        'Dean Oliver game ratings in the game-rating log. Zero-rating games are included; no minute weighting, '
        'trimming, or game removal is applied. AVG Net is the simple average of game Net ratings.',
...
def page_player_profiles():
    ...
    for idx, name in enumerate(profiled):
        ...
        # Player role label
        usage_pct = adv.get('usage_pct', 0)
        role_label, role_color = player_role_from_poss(usage_pct, mpg)
        ...
        p_hdr = Table([[
            ...
            Paragraph(
                ...
                f'<font color="#888888">Avg ORtg {ortg_val:.1f}  ·  Avg DRtg {drtg_val:.1f}  ·  </font>'
        ...
        if top100_flags:
            narrative = blend_top100_into_narrative(narrative, top100_flags)
        e.append(Paragraph(narrative, S(8.5, space_after=4)))
        ...
        if season_min_pct >= 30 and ind_ortg_v > 0:
            _gp = max(avg.get('games', tot.get('games', 0)), 1)
            _twoa = max(tot.get('FGA', 0) - tot.get('3PA', 0), 0)
            _profile_rank_row = {
                'team': 'Moorpark',
                'gp': tot.get('games', avg.get('games', 0)),
                'min_pct': season_min_pct,
                'ind_ortg': ind_ortg_v,
                'ind_drtg': ind_drtg_v,
                'usage_pct': usage_v,
                'ast_rate': ast_v,
                ...
                'tpa_pg': avg.get('3PA', 0),
                'fta_pg': avg.get('FTA', 0),
                'fga_pg': tot.get('FGA', 0) / _gp,
                'shooting_attempts_pg': (tot.get('FGA', 0) + tot.get('FTA', 0)) / _gp,
            }

            def _adv_piece(label, key, value, hb=True, suffix='%', show_when_zero=False):
                if value <= 0 and not show_when_zero:
                    return ''
                rank_text = ''
                if adv_cat_eligible(_profile_rank_row, key):
                    _rank, _total = adv_rank_qualified(key, value, hb)
                    if _rank > 0 and _total > 0:
                        rank_text = f' (#{_rank})'
                return f'{label} <b>{value:.1f}{suffix}</b>{rank_text}'

            _pieces = [
                _adv_piece('Ind ORtg', 'ind_ortg', ind_ortg_v, True, ''),
                _adv_piece('Ind DRtg', 'ind_drtg', ind_drtg_v, False, ''),
                _adv_piece('%Poss', 'usage_pct', usage_v, True, '%'),
                _adv_piece('AST Rate', 'ast_rate', ast_v, True, '%'),
                _adv_piece('TOV%', 'tov_pct', tov_v, False, '%'),
                _adv_piece('OREB%', 'oreb_pct', oreb_v, True, '%'),
                _adv_piece('DREB%', 'dreb_pct', dreb_v, True, '%'),
                _adv_piece('STL%', 'stl_pct', stl_v, True, '%'),
                _adv_piece('BLK%', 'blk_pct', blk_v, True, '%'),
                _adv_piece('FT Rate', 'ft_rate', ftr_v, True, ''),
                _adv_piece('2P%', 'twop', twop_v, True, '%'),
                _adv_piece('3P%', 'tpp', tpp_v, True, '%'),
                _adv_piece('FT%', 'ftp', ftp_v, True, '%'),
            ]
            adv_prose = '<i>Advanced analytics: ' + '  ·  '.join(p for p in _pieces if p) + '</i>'
...
# ═══════════════════════════════════════════════════════════════════════════════
def main():
    out = f'{BASE}/moorpark_scouting_report_2025_26_top100_onepara_copy.pdf'
