...
Reads box scores from '2017-18 Teams Schedules/<Team>/' and writes
aggregated stats to '2017-18 Team Statistics/<Team>/'.

Usage:
    python generate_team_stats_2017.py --teams Moorpark Ventura
    python generate_team_stats_2017.py --all
"""
...
SCHEDULES_DIR = Path("2017-18 Teams Schedules")
STATS_DIR = Path("2017-18 Team Statistics")
...
TEAM_ALIASES = {
    ...
    # Additional display-name variants found in 2017-18 box scores
    "COPPERMO": "Copper Mountain",
    "Copper Mtn.": "Copper Mountain",
    "EASTLOSA": "East Los Angeles",
    "East L.A. Huskies": "East Los Angeles",
    "ELCAMINO": "El Camino",
    "L.A. Pierce Brahmas": "LA Pierce",
    "LAHARBOR": "LA Harbor",
    "LAPIERCE": "LA Pierce",
    "LASOUTHW": "LA Southwest",
    "LATRADET": "LA Trade Tech",
    "LAVC Monarchs": "LA Valley",
    "LONGBEACH": "Long Beach",
    "Los Angeles Harbor": "LA Harbor",
    "MT SAN JACINTO": "Mt. San Jacinto",
    "MTSANJAC": "Mt. San Jacinto",
    "Mt.S. Jacinto Eagles": "Mt. San Jacinto",
    "Mount San Antonio": "Mt. San Antonio",
    "ORANGECOAST": "Orange Coast",
    "ORANGECO": "Orange Coast",
    "Pasadena Lancers": "Pasadena City",
    "PASADE~1": "Pasadena City",
    "RIOHONDO": "Rio Hondo",
    "Riverside Tigers": "Riverside",
    "S.Bernardino Wolverines": "San Bernardino Valley",
    "SANDIEGO": "San Diego Mesa",
    "SANTAANA": "Santa Ana",
    "SANTAMON": "Santa Monica",
    "SD MESA": "San Diego Mesa",
    "San Bernardino Vy.": "San Bernardino Valley",
    "IRVINEVA": "Irvine Valley",
    "MTSAC(~1": "Mt. San Antonio",
    "Ventura Pirates": "Ventura",
    "WESTLAW": "West LA",
    "West Hills-Coalinga": "West Hills Coalinga",
    "West L.A. Wildcats": "West LA",
}
...
def _resolve_team_name(name: str) -> str:
    ...
    for team_name in _canonical_teams():
        ...
        if normalized_name == normalized_team or normalized_name.startswith(normalized_team + " "):
            return team_name
    return name
...
def _game_minutes(bs: dict) -> int:
    ...
    for team_players in bs.get("teams", {}).values():
        team_min = sum(
            ...
            and str(p.get("stats", {}).get("MIN", "0")).lstrip("-").isdigit()
        )
        total_min += team_min
        count += 1
    if count == 0:
...
def aggregate_player_stats(team_name: str, boxscores: list[dict]) -> dict:
    ...
    for bs in boxscores:
        ...
        for player in bs["teams"][team_key]:
            ...
            try:
                minv = str(s.get("MIN", "0"))
                p["MIN"] += int(minv) if minv.lstrip("-").isdigit() else 0
...
def compute_game_log(team_name: str, boxscores: list[dict], schedule: dict) -> list[dict]:
    ...
    for bs in boxscores:
        ...
        # Some 2017-18 box scores contain broken score fields (e.g., 0 with valid player totals).
        # Prefer schedule scores when box-score score fields are broken, then fall back
        # to summed player points if the schedule record is also missing.
...
def main():
    parser = argparse.ArgumentParser(description="Generate 2017-18 team statistics from box scores")
