...
Reads box scores from '2025-26 Teams Schedules/<Team>/' and writes
aggregated stats to '2025-26 Team Statistics/<Team>/'.

Usage:
    python generate_team_stats.py --teams Moorpark Ventura
    python generate_team_stats.py --all
"""
...
from ccc_config import DEFAULT_SEASON, schedule_dir, stats_dir

SCHEDULES_DIR = schedule_dir(DEFAULT_SEASON)
STATS_DIR = stats_dir(DEFAULT_SEASON)
...
def normalize_player_name(name: str) -> str:
    """Normalize player name: '#03 Smith' / 'g 03 Smith' / 'F 3 Smith' / 'C 5 Smith' -> '#3 Smith'."""
...
def _player_merge_key(name: str) -> str:
    """Strip jersey number to get a merge key: '#23 Dylan Griffin' -> 'Dylan Griffin'."""
...
def _game_minutes(bs: dict) -> int:
    ...
    Uses the stored 'game_minutes' field if available (from newer scrapes),
    otherwise detects OT by averaging both teams' total player-minutes.
    In any basketball game exactly 5 players are on the court at all times,
    so total player-minutes per team ≈ 5 × game_minutes.
    Regular game → ~200, single OT → ~225, double OT → ~250, etc.
    """
    # Prefer explicit field from scraper
    if "game_minutes" in bs:
        return bs["game_minutes"]
    # Fallback: heuristic from player minutes
    def _safe_min(p):
        # A few scraped DNP rows carry "0-0" spillover in the MIN field
        try:
            return int(p.get("stats", {}).get("MIN", "0"))
        except ValueError:
            return 0
...
def aggregate_opponent_stats(team_name: str, boxscores: list[dict]) -> dict:
    """Aggregate opponent totals across all games (what opponents did against this team)."""
    season = {
        "games": 0,
        "FGM": 0, "FGA": 0, "3PM": 0, "3PA": 0, "FTM": 0, "FTA": 0,
        "OREB": 0, "DREB": 0, "REB": 0, "AST": 0, "STL": 0, "BLK": 0,
    ...
    for bs in boxscores:
        ...
        if not opp_key:
            continue
        # Skip games where the team side has no players (empty roster)
        team_key = _find_team_key(team_name, bs)
        if not team_key or not bs["teams"].get(team_key):
            continue
        game_totals = _sum_team_box(bs["teams"][opp_key])
        # Skip games where opponent reported no real stats (all zeroes)
...
def aggregate_player_stats(team_name: str, boxscores: list[dict]) -> dict:
    ...
    for bs in boxscores:
        # Find the team's player list in this box score
        team_key = _find_team_key(team_name, bs)
        if not team_key:
            continue
...
def compute_team_totals(player_stats: dict, boxscores: list[dict] | None = None) -> dict:
    ...
    MIN is computed as actual game-clock minutes (40 + 5 per OT) rather
    than the sum of individual player minutes.
    """
    totals = {
        "games": 0, "MIN": 0,
        "FGM": 0, "FGA": 0, "3PM": 0, "3PA": 0, "FTM": 0, "FTA": 0,
    ...
    # MIN = actual game-clock minutes (40 regular + 5 per OT)
    if boxscores:
...
def compute_game_log(team_name: str, boxscores: list[dict], schedule: dict) -> list[dict]:
    ...
    # Build schedule lookup by normalized (month, day) key
    schedule_map = {}
    ...
    def _bs_date_key(date_str):
        """Convert box score date 'M/D/YYYY' to (month, day) tuple."""
    ...
    def _resolve_alias(name):
        """Resolve a scraped team label to a canonical schedule name."""
        return _resolve_team_name(name)

    def _is_team(name):
        """Check if a name (or its alias) matches team_name."""
    ...
    for bs in boxscores:
        ...
        # Determine opponent (resolve aliases before comparing)
        ...
        # Score (resolve aliases before matching)
        scores = bs.get("score", {})
        team_score = 0
        opp_score = 0
        ...
        # Game duration (40 regulation + 5 per OT)
        entry["MIN"] = _game_minutes(bs)

        # Conference flag and location from schedule
        ...
        # Home/Away already set above; only override if schedule says neutral

        # Per-game box score totals for team and opponent
        team_key = _find_team_key(team_name, bs)
        opp_key = _find_opponent_key(team_name, bs)
        if team_key:
...
def generate_stats(team_name: str, conference: str = None):
    ...
    for game_dir in sorted(team_sched_dir.iterdir()):
        ...
        for bs_file in game_dir.glob("*.json"):
            with open(bs_file) as f:
                bs = json.load(f)
            # Override date from folder name (YYYYMMDD_opponent) — more reliable than scraped date field
            folder_date_str = game_dir.name[:8]
            if len(folder_date_str) == 8 and folder_date_str.isdigit():
                y, m, d = folder_date_str[:4], folder_date_str[4:6], folder_date_str[6:8]
                bs["date"] = f"{int(m)}/{int(d)}/{y}"
            boxscores.append(bs)
    ...
    # Build schedule lookup for conference flag
    month_names = {"jan": 1, "feb": 2, "mar": 3, "apr": 4, "may": 5, "jun": 6,
                   "jul": 7, "aug": 8, "sep": 9, "oct": 10, "nov": 11, "dec": 12}
    schedule_map = {}
    for g in schedule.get("games", []):
    ...
    # Player season totals with averages
    player_list = []
    ...
    # Opponent totals and averages
    opp_totals = aggregate_opponent_stats(team_name, boxscores)
    # Opponent MIN = same game-clock minutes (both teams play the same game)
    ...
    # Filter out games with bad box score data:
    #   - team TO = 0 (scraping artifact; real teams always turn the ball over)
    #   - opp STL > team TO (statistically impossible; steals are a subset of turnovers)
    def _game_team_to(bs):
        tkey = _find_team_key(team_name, bs)
        return _sum_team_box(bs["teams"].get(tkey, [])).get("TO", 0) if tkey else 0
    def _game_opp_stl(bs):
        okey = _find_opponent_key(team_name, bs)
        return _sum_team_box(bs["teams"].get(okey, [])).get("STL", 0) if okey else 0
    conf_boxscores = [bs for bs in conf_boxscores
                      if _game_team_to(bs) > 0 and _game_opp_stl(bs) <= _game_team_to(bs)]
    ...
    # Conference record from schedule
    conf_record = {}
...
def main():
    global SCHEDULES_DIR, STATS_DIR, _CANONICAL_TEAMS
    parser = argparse.ArgumentParser(description="Generate team statistics from box scores")
    ...
    SCHEDULES_DIR = schedule_dir(args.season)
    STATS_DIR = stats_dir(args.season)
    _CANONICAL_TEAMS = None
