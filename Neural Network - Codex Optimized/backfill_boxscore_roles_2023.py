"""Backfill starter/reserve roles into saved 2023-24 box score JSON files."""
...
from scrape_team_2023 import fetch, parse_boxscore


BASE_DIR = Path("2023-24 Teams Schedules")
PROGRESS_PATH = Path("outputs/bench_role_backfill_progress_2023_24.json")
REPORT_PATH = Path("outputs/bench_role_backfill_report_2023_24.json")


def game_id_for(date_str: str, opponent: str) -> str:
    dt = datetime.strptime(date_str.strip() + " 2023", "%b %d %Y")
    if dt.month <= 6:
...
def main() -> None:
    ...
    print(f"ready team-game files skipped: {skipped_ready}", flush=True)
    print(f"unique URLs needing refresh: {total_urls}", flush=True)
    print(f"previously completed URLs in checkpoint: {len(completed_urls)}", flush=True)
