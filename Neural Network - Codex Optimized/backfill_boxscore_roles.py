"""Backfill starter/reserve roles into saved 2025-26 box score JSON files.

The original box score parser skipped the STARTERS and RESERVES section labels.
After updating scrape_team.parse_boxscore to preserve those labels, this script
refreshes the saved per-game JSON without re-scraping team schedule pages.
"""
...
from scrape_team import fetch, parse_boxscore


BASE_DIR = Path("2025-26 Teams Schedules")
PROGRESS_PATH = Path("outputs/bench_role_backfill_progress.json")
REPORT_PATH = Path("outputs/bench_role_backfill_report.json")


def game_id_for(date_str: str, opponent: str) -> str:
    dt = datetime.strptime(date_str.strip() + " 2025", "%b %d %Y")
    if dt.month <= 6:
