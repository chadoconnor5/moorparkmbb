...
function showGamePlan(teamName) {{
  ...
  games.forEach(gr => {{
    ...
    rowsHtml += `<tr class="${{rowCls}}"><td style="text-align:left;white-space:nowrap;font-size:0.77rem">${{gr.date||''}}</td><td style="text-align:center">${{rk}}</td><td style="text-align:left">${{oppLink}}</td><td style="text-align:center">${{resDisplay}}</td>${{cells}}</tr>`;
  }});
